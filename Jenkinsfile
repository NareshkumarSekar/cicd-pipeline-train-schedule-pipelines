pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("nareshkumarsekar/train-schedule")
                    app.inside {
...
stage ('DeployToProduction') {
    ...
    steps {
        ...
        withCredentials ([usernamePassword(credentialsId: 'webserver_login', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
            script {
                sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@${env.prod_ip} \"docker pull nareshkumarsekar/train-schedule:${env.BUILD_NUMBER}\""
                try {
                ...
                sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@${env.prod_ip} \"docker run --restart always --name train-schedule -p 8080:8080 -d nareshkumarsekar/train-schedule:${env.BUILD_NUMBER}\""
            }
        }
    }
}
